...
def parse_joint(bvh, token_index, skeleton, bone_context, non_end_bones,
                motion_channels):
    # print bone_context
    end_site = False
    ...
    while (bvh[token_index][0] == "IDENT" and bvh[token_index][1] == "JOINT") or \
        ...
        (token_index, skeleton,
         bone_context, non_end_bones, motion_channels) = parse_joint(
             bvh, token_index, skeleton, bone_context, non_end_bones,
             motion_channels)
...
def parse_hierarchy(bvh_tokens):
    # global current_token
    current_token = 0
    if bvh_tokens[current_token] != ("IDENT", "HIERARCHY"):
        return None
    current_token = current_token + 1
    if bvh_tokens[current_token] != ("IDENT", "ROOT"):
        return None
    current_token = current_token + 1
    if bvh_tokens[current_token][0] != "IDENT":
        return None
    root_name = bvh_tokens[current_token][1]
    root_bone = new_bone(None, root_name)
    current_token = current_token + 1
    if bvh_tokens[current_token][0] != "OPEN_BRACE":
        return None
    current_token = current_token + 1
    offsets, current_token = read_offset(bvh_tokens, current_token)
    channels, current_token = read_channels(bvh_tokens, current_token)
    ...
    while bvh_tokens[current_token][1] == "JOINT":
        (current_token, skeleton,
         bone_context, non_end_bones, motion_channels) = parse_joint(
             bvh_tokens, current_token, skeleton, bone_context, non_end_bones,
             motion_channels)
