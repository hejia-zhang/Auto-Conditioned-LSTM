...
def get_hip_transform(motion, skel):
    ...
    Yrotation = motion[4]
    Xrotation = motion[5]
    theta = [Xrotation, Yrotation, Zrotation]
...
def get_pos(bone, motion, non_end_bones, skel):
    global_transform = np.dot(
        get_hip_transform(motion, skel),
        get_global_transform(bone, skel, motion, non_end_bones))
...
def get_global_transform(bone, skel, motion, non_end_bones):
    ...
    Transformation = get_relative_transformation(bone, non_end_bones, motion,
                                                 skel)
    while parent != None:
        parent_transformation = get_relative_transformation(
            parent, non_end_bones, motion, skel)
...
def visualize_joints(bone_list, focus):
    ...
    for bone in bone_list:
        ...
        p2y = bone[1][1]
        p2z = bone[1][2] + 400
        p1 = (int(p1x * focus / p1z + 300.0), int(-p1y * focus / p1z + 204.0))
...
def xyz_to_rotations(skel, position):
    ...
    for bone in skel.keys():
        if bone != 'hip':
            ...
            displacement_normalized = displacement / np.linalg.norm(
                displacement)
            orig_offset = np.array(skel[bone]['offsets'])
            orig_offset_normalized = orig_offset / np.linalg.norm(orig_offset)
            rotation = rel_rotation(orig_offset_normalized,
                                    np.transpose(displacement_normalized))
            all_rotations[parent] = rotationMatrixToEulerAngles(rotation) * (
                180 / math.pi)
    ...
    while len(children_dict.keys()) - 1 > len(all_rotation_matrices.keys()):
        for bone in children_dict.keys():
            ...
            for i in range(len(children)):
                children_xyz[i, :] = np.array(
                    position[children[i]]) - np.array(position[bone])
                ...
                assert np.allclose(
                    np.linalg.norm(children_xyz[i, :]),
                    np.linalg.norm(children_orig[i, :]))
            ...
            if bone == 'hip':
                ...
                angles = np.array(euler.mat2euler(rotation, 'syxz')) * (
                    180.0 / math.pi)
                all_rotations[bone] = [angles[1], angles[0], angles[2]]
